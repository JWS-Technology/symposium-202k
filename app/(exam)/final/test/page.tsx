...
export default function SpiderTestRoom() {
    ...
    /* =================================================
       🚀 UPDATED SUBMISSION (USING COOKIES)
       ================================================= */
    const submitTest = async () => {
        ...
        try {
            // 2. DECODE DATA (Base64 -> JSON)
            // The cookie contains the stringified user object we encoded in the login page
            ...
            const submissionPayload = {
                ...
                violations: violations, // Send violation count for auditing
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <header className="mb-8">
                                ...
                                <h2 className="text-2xl md:text-3xl font-black text-white leading-tight mt-4 italic">
                                    {parsed.text}
                                </h2>
                            </header>
                            ...
                            {parsed.code && (
                                <div className="mb-8 rounded-3xl overflow-hidden border-2 border-zinc-800 bg-black/80">
                                    ...
                                    <pre className="p-6 text-sm md:text-lg font-mono text-cyan-400 overflow-x-auto">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {currentQ.options.map((opt, i) => (
                                    <motion.button
                                        key={i}
                                        whileHover={{ x: 10 }}
                                        onClick={() => setAnswers(prev => ({ ...prev, [currentQ._id]: i }))}
                                        className={`group w-full p-6 rounded-2xl border-2 transition-all text-left flex items-center justify-between
                      ${answers[currentQ._id] === i ? 'border-red-600 bg-red-600/10 text-white' : 'border-zinc-800 bg-zinc-900/40 text-zinc-400 hover:border-zinc-700'}`}
                                    >
                                        <div className="flex items-center gap-6">
                                            <span className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-black border-2
                        ${answers[currentQ._id] === i ? 'bg-red-600 border-red-400 text-white' : 'bg-black border-zinc-700 text-zinc-500'}`}>
                                                {String.fromCharCode(65 + i)}
                                            </span>
                                            <span className="text-lg md:text-xl font-bold">{opt}</span>
                                        </div>
                                        {answers[currentQ._id] === i && <FaCheckCircle className="text-red-600 text-2xl shadow-[0_0_10px_red]" />}
                                    </motion.button>
                                ))}
                            </div>
                        </motion.div>
            ...
            <footer className="border-t-2 border-zinc-900 p-8 bg-black/95 mt-auto">
                <div className="max-w-4xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        {questions.map((_, i) => (
                            <div key={i} className={`h-1.5 rounded-full transition-all ${i === currentIndex ? 'bg-red-600 w-8' : answers[questions[i]._id] !== undefined ? 'bg-red-900 w-3' : 'bg-zinc-800 w-3'}`} />
                        ))}
                    </div>
