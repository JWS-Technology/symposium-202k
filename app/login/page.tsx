        ...
        >
            <style jsx global>{`
                ...
                .stark-input-container {
                    ...
                    /* Removed sharp clip-path for elegant rounded corners */
                    border-radius: 12px;
                ...
                .stark-input-container:focus-within {
                    ...
                    box-shadow: 0 10px 30px -5px rgba(255, 0, 0, 0.4);
            ...
            {QUOTES.map((q, i) => (
                <div key={i} className="animate-float absolute z-20 hidden xl:block p-4 bg-black/90 border-l-4 border-red-600 backdrop-blur-3xl rounded-lg shadow-2xl"
                    style={{ left: q.side === "left" ? "5%" : "auto", right: q.side === "right" ? "5%" : "auto", top: `${20 + (i * 20)}%`, animationDelay: `${i * 1.5}s` }}>
                    <div className="flex items-center gap-2 mb-1">
                        <Globe className="w-3 h-3 text-red-600 animate-spin-slow" />
                        <span className="text-[9px] text-red-600 font-black tracking-[0.2em] uppercase">LINK_ESTABLISHED</span>
                    </div>
                    <p className="text-white text-xs font-bold max-w-[200px] leading-snug italic">"{q.text}"</p>
                </div>
            ))}

            {/* COMPACT CRT ACCESS PORTAL */}
            <div className={`relative z-40 w-full max-w-[420px] transition-all duration-500 ${isGlitching ? 'translate-x-1 skew-y-0.5' : ''}`}>
                <div className="relative bg-[#050505]/95 border border-white/20 p-8 md:p-10 rounded-[2rem] shadow-[0_0_100px_rgba(0,0,0,1)] overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex flex-col items-center mb-8 text-center">
                            <div className="mb-4 relative group">
                                ...
                                <Target className={`relative w-12 h-12 transition-all duration-300 ${isGlitching ? 'text-blue-500 scale-110' : 'text-red-600'}`} />
                            </div>
                            ...
                            <div className="h-[2px] w-12 bg-red-600 mt-4 mb-2" />
                            <p className="text-[10px] text-gray-400 font-black tracking-[0.5em] uppercase">Auth Terminal</p>
                        </div>
                        ...
                        <form className="space-y-6" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-center px-1">
                                    ...
                                    <span className="text-[8px] text-red-900 font-black tracking-widest animate-pulse">SCANNING...</span>
                                </div>
                                <div className="stark-input-container flex items-center px-5 py-4">
                                    <User className="w-4 h-4 text-red-700 mr-4 opacity-50" />
                                    <input
                                        type="text"
                                        placeholder="AGENT_ID"
                                        className="stark-field bg-transparent border-none outline-none text-md w-full focus:ring-0 uppercase"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="stark-input-container flex items-center px-5 py-4">
                                    <Lock className="w-4 h-4 text-red-700 mr-4 opacity-50" />
                                    <input
                                        type={showPassword ? "text" : "password"}
                                        placeholder="••••••••"
                                        className="stark-field bg-transparent border-none outline-none text-md w-full focus:ring-0"
                                    />
                                    <button
                                        type="button"
                                        onClick={() => setShowPassword(!showPassword)}
                                        className="ml-2 hover:scale-110 transition-all active:scale-90"
                                    >
                                        {showPassword ? (
                                            <EyeOff className="w-5 h-5 text-red-600 drop-shadow-[0_0_10px_rgba(255,0,0,0.8)]" />
                                        ) : (
                                            <Eye className="w-5 h-5 text-red-800 opacity-40 hover:opacity-100" />
                                        )}
                                    </button>
                                </div>
                            </div>
                            ...
                            {/* BUTTON */}
                            <button className="w-full relative mt-6 group overflow-hidden rounded-xl active:scale-[0.96] transition-all duration-300">
                                ...
                                <div className="relative border-2 border-red-600 py-5 rounded-xl flex items-center justify-center gap-3 group-hover:border-white/20 shadow-lg">
                                    <ShieldCheck className="w-5 h-5 text-red-600 group-hover:text-white transition-colors duration-300" />
                                    <span className="text-white font-black uppercase tracking-[0.4em] text-xs group-hover:scale-105 transition-transform">
                                        Initialize
                                    </span>
                                </div>
                            </button>
                        ...
                        <div className="mt-10 flex flex-col items-center gap-3 opacity-40 hover:opacity-100 transition-all duration-700">
                            <div className="flex gap-2">
                                {[...Array(3)].map((_, i) => (
                                    <div key={i} className="w-1 h-1 bg-red-600 rotate-45 animate-pulse" style={{ animationDelay: `${i * 150}ms` }} />
                                ))}
                            </div>
                            <p className="text-[8px] text-white font-black tracking-[0.4em] uppercase opacity-60">Stark Industries © 2026</p>
                        </div>
                    </div>
                </div>
            </div>
